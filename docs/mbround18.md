...
| [vtt-maps-portal](https://github.com/mbround18/vtt-maps-portal)                                 | 🚧     | Not `gh-reusable`-based — hand-rolled `ci.yaml` (`rust`/`web`/`docker` jobs); `docker` already used `paws-up`/`paws docker`. Converting `rust`/`web` → `paws ci --toolchain rust`/`node` needed a real `paws` fix first: `apps/web` has its own `build`/`test`/`lint` scripts *and* a Playwright e2e suite in one `package.json`, but `paws ci --toolchain node` used to treat Playwright detection as exclusive of the normal build/test/lint pipeline instead of additive to it — fixed in `paws-node`'s `dagger_pipeline_args`. Conversion PR open. |
