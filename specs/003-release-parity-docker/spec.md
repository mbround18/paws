...
- **Changelog Entry**: one dated, version-headed section appended to `CHANGELOG.md`, containing one line per pull request in the resolved range (rendered as its title, however many of its commits are in range) plus one line per commit with no pull request (rendered as its raw subject, the fallback per FR-009). Commits carrying `[skip ci]` or `[ci skip]` are left out.
