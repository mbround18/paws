...
- [X] T018 [P] [US1] Add `examples/rust-coverage-fixture/` (a crate with one deliberately untested branch) and a `paws ci --toolchain rust --coverage` step against it in `.github/workflows/ci.yaml`'s `ci-e2e` job, asserting the reported coverage is genuinely between 0% and 100% — proving the tool measures a real gap, not a fixed number. Originally written as a `crates/paws-rust/tests/e2e_coverage.rs` `#[test]` shelling out to `docker` directly (mirroring `paws-docker`'s own `tests/e2e_docker_daemon.rs`), but that violates `scripts/check-dagger-callsites.sh`'s SC-004/ADR-0001 lint (`docker`/`cross`/`dagger` spawn call sites are forbidden outside `crates/paws-dagger`, with the sole exception of `crates/paws-docker/tests/`) — caught by CI, not by local `cargo test --workspace` (the lint is a separate `./scripts/check-dagger-callsites.sh` CI step, not itself a Rust test). Moved into the CI workflow instead, which reaches `docker` only indirectly through the real `dagger` CLI/engine, same as every other `paws ci` invocation
