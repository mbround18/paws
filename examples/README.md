...
- `ruby-fixture/` — a minimal Bundler project (`Gemfile` + committed `Gemfile.lock`, a `Rakefile`
  wiring `Rake::TestTask`, a `Calculator` module, and a Minitest suite); the target for
  `paws ci --toolchain ruby`. Exercises `crates/paws-ruby`'s Gemfile detection, its Rake-over-RSpec
  test-runner choice, and the `BUNDLE_FROZEN=true` lockfile path — `bundle install` plus
  `bundle exec rake` genuinely running 2 assertions against `ruby:trixie` through Dagger.
- `php-fixture/` — a minimal Composer project (`composer.json` with PSR-4 autoloading and a
  PHPUnit dev-dependency, `phpunit.xml`, a `Calculator` class, and a `TestCase` suite); the target
  for `paws ci --toolchain php`. Exercises `crates/paws-php`'s `composer.json` detection and its
  PHPUnit-config gate — `composer validate --strict`, `composer install`, and a real
  `vendor/bin/phpunit` run (PHPUnit 11 on PHP 8.5) against `composer:2` through Dagger.
- `dotnet-fixture/` — a real `dotnet new sln`/`classlib`/`xunit` scaffold (generated by
  `mcr.microsoft.com/dotnet/sdk:10.0` itself, not hand-written) with a `Calculator` library under
  `src/` and an xUnit project referencing it under `tests/`; the target for
  `paws ci --toolchain dotnet`. Exercises `crates/paws-dotnet`'s solution detection — note the SDK
...
--warnings-as-errors` → `test` chain against `elixir:otp-28` through Dagger. Deliberately
  dependency-free: the Hex/rebar install steps exist so a project *with* dependencies doesn't hit
  Mix's interactive install prompt, and this fixture proves they're harmless no-ops without any.
